...
if __name__ == '__main__':
    ...
    kp_detector = build_kp_detector('./checkpoint/vox-cpk.pth.tar',**config['model_params']['kp_detector_params'],**config['model_params']['common_params'])
    generator = build_generator('./checkpoint/vox-cpk.pth.tar', **config['model_params']['generator_params'],**config['model_params']['common_params'])
    ...
    reader = imageio.get_reader('driving.mp4')
    ...
    driving_video = [resize(frame, (256, 256))[..., :num_channels] for frame in driving_video][0:len(driving_video) if len(tf.config.list_physical_devices('GPU'))>0 else 40]
    ...
    imageio.mimsave('model_test.mp4',[img_as_ubyte(frame) for frame in predictions], fps=fps)
    print('Done.')
