...
def build(checkpoint_path, config_path, output_name, module):
...
parser.add_argument('--module', choices=['all', 'kp_detector', 'generator', 'process_kp_driving'], default='all',
...
if not parser.a:
    ...
    build(checkpoint_path, config_path, output_name, parser.module)
else:
    ...
    for i, config in enumerate(tqdm(configs)):
        build(checkpoints[i], config, output_names[i], parser.module)
